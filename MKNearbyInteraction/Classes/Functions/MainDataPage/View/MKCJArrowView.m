...
typedef NS_ENUM(NSInteger, MKCJAnimationType) {
    MKCJAnimationType_arc,      //圆弧
    MKCJAnimationType_point,    //缩放
};

@interface MKCJArrowView ()
...
@property (nonatomic, assign)MKCJAnimationType animationType;

@end

@implementation MKCJArrowView
...
- (void)drawRect:(CGRect)rect {
    if (self.animationType == MKCJAnimationType_arc) {
        //默认为圆弧
...
- (void)setArrowRotationAngle:(CGFloat)angle {
    ...
    if (smallAngle && (self.animationType == MKCJAnimationType_arc)) {
        //需要圆点缩放
        [self setNeedsDisplay];
        [self startAnimation];
    }else if (!smallAngle && (self.animationType == MKCJAnimationType_point)) {
        //需要圆弧
        [self stopAnimation];
    }
    
    self.animationType = (smallAngle ? MKCJAnimationType_point : MKCJAnimationType_arc);
...
- (void)setupArrowImageView {
    ...
    self.arrowIcon.image = LOADICON(@"MKNearbyInteraction", @"MKCJArrowView", @"cj_white_Arrow.png");
